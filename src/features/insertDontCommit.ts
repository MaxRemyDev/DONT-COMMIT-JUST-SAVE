...
export function registerInsertDontCommit(context: vscode.ExtensionContext): void {
    const disposable = vscode.commands.registerCommand('extension.insertDontCommit', async () => {
        const git = await getGitApi();
        if (!git) { return; }

        const repo = await pickRepository(git.repositories, {
            title: 'Pick repo',
            placeHolder: 'Which repo to insert the marker?'
        });
